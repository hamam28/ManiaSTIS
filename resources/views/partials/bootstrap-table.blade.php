    ...
    function genericActionsFormatter(owner_name, element_name) {
        ...
        return function (value,row) {
            ...
            if ((row.available_actions) && (row.available_actions.delete === true)) {
                ...
                var delete_message = "{{ trans('general.sure_to_delete') }} ";

                if(!row.name) {
                    delete_message += "transaksi dari " + row.company_id.name + " pada tanggal " + row.purchase_date.date;
                } else {
                    delete_message += row.name;
